...
public class NotPersonalGroupAttribute : Attribute, IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (isPersonalGroup)
        {
            context.Result = new BadRequestObjectResult(new { message = Constants.ErrorMessages.CannotPerformActionOnPersonalGroups });
            return;
        }
